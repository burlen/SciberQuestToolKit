...
JID=`qsub -v MPI_TYPE_MAX,MPI_GROUP_MAX,PV_PATH,PATH,LD_LIBRARY_PATH,PV_NCPUS,CONFIG_FILE,BOV_FILE,OUTPUT_PATH,START_TIME,END_TIME,PV_EXE -N svtk-batch -A $ACCOUNT -q $QUEUE -l ncpus=$NCPUS,mem=$MEM\MB,walltime=$WALLTIME /sw/analysis/paraview/3.12.0/sles11.1_intel11.1.038/bin/qsub-batch-nautilus.qsub`
ERRNO=$?
if [ $ERRNO == 0 ] 
then

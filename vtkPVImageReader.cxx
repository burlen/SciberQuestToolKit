/*=========================================================================
  ...
  Module:    $RCSfile: vtkPVImageReader.cxx,v $
...
vtkCxxRevisionMacro(vtkPVImageReader, "$Revision: 1.122 $");
vtkStandardNewMacro(vtkPVImageReader);

vtkCxxSetObjectMacro(vtkPVImageReader,Transform,vtkTransform);
...
/// note: addeed for testing purposes
#if defined vtkPVImageReaderTIME
int vtkPVImageReader::CanReadFile(const char* fname)
{
  size_t n=strlen(fname);
  if (fname[n-3]=='g' && fname[n-2]=='d' && fname[n-1]=='a')
    {
    return 1;
    }
  return 0;
...
int vtkPVImageReader::RequestInformation (
...
int vtkPVImageReader::OpenAndSeekFile(int dataExtent[6], int idx)
{
...
void vtkImageReaderUpdate2(vtkPVImageReader *self, vtkImageData *data,
                           IT *inPtr, OT *outPtr)
{
...
// templated to handle different data types.
template <class T>
void vtkImageReaderUpdate1(vtkPVImageReader *self, vtkImageData *data, T *inPtr)
{
  void *outPtr;
...
void vtkPVImageReader::ExecuteData(vtkDataObject *output)
{
  #ifdef vtkPVImageReaderTIME
  double walls=0.0;
  timeval wallt;
  if (this->ProcId==0)
    {
  ...
  if (this->ProcId==0)
    ...
    cerr << "vtkPVImageReader::RequestData " << walle-walls << endl;
    }
  #endif
}
...
void vtkPVImageReader::ComputeTransformedSpacing (double Spacing[3])
{
  if (!this->Transform)
    {
...
void vtkPVImageReader::ComputeTransformedOrigin (double origin[3])
{
  if (!this->Transform)
    {
...
void vtkPVImageReader::ComputeTransformedExtent(int inExtent[6],
                                              int outExtent[6])
{
...
void vtkPVImageReader::ComputeInverseTransformedExtent(int inExtent[6],
                                                     int outExtent[6])
{
  double transformedExtent[3];
  int temp;
...
void vtkPVImageReader::ComputeTransformedIncrements(vtkIdType inIncr[3],
                                                  vtkIdType outIncr[3])
{
...
void vtkPVImageReader::ComputeInverseTransformedIncrements(vtkIdType inIncr[3],
                                                         vtkIdType outIncr[3])
{
  double transformedIncr[3];

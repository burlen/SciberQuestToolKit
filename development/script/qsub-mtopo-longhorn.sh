...
if [ -z $1 ]
then
  echo "Error: \$2 should be the start time."
  exit
fi
START_TIME=$1
shift 1
...
if [ -z $1 ]
then
  echo "Error: \$3 should be the end time."
  exit
fi
END_TIME=$1
...
$IBRUN_PATH/ibrun MagnetosphereTopoBatch $CONFIG_FILE $START_TIME $END_TIME
